 ...
 * - ?scope=jobs: cron/reminder/recurring jobs + summary
 *
 * Proxies to OpenClaw gateway `sessions.list` RPC.
 */

import { tryGatewayRpc, getGatewayConfig } from './_lib/gateway.js'
import { getMockSessions, getMockJobsData } from './_lib/mock.js'
...
function inferCadence(input) {
  ...
  if (text.includes('@hourly') || text.includes('hourly') || /\*\s+\*\s+\*\s+\*\s+\*/.test(text)) return 'hourly'
  ...
  if (text.includes('every day')) return 'daily'
  if (text.includes('every week')) return 'weekly'
  if (text.includes('every month')) return 'monthly'
...
function normalizeLastRunStatus(rawStatus, enabled) {
...
function normalizeJob(session, now) {
  ...
  return {
    ...
    cadence,
    nextRunMs: deriveNextRunMs({ ...session, cadence }, now),
    lastRunMs,
    lastRunStatus: normalizeLastRunStatus(rawLastStatus, enabled),
    enabled,
    owner: owner || null,
    target: session.target || session.channel || session.schedule?.target || null,
  }
}
...
function buildJobsSummary(jobs, now) {
  ...
  const failedOrRecentIssueCount = jobs.filter((job) => {
...
export default async function handler(req, res) {
  ...
  if (isMock) {
    if (scope === 'jobs') {
      ...
      const jobs = applyJobFilters(mock.jobs, req.query)
      return jsonResponse(res, 200, {
        jobs,
        summary: buildJobsSummary(mock.jobs, now),
        total: jobs.length,
  ...
  try {
    ...
    if (scope === 'jobs') {
      ...
      const jobs = applyJobFilters(allJobs, req.query)

      return jsonResponse(res, 200, {
        jobs,
        summary: buildJobsSummary(allJobs, now),
        total: jobs.length,
